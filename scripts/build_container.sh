...
do
    ...
    if [ $IMG == "jupyter-server" ]; then
        ...
        build=(docker build --platform linux/amd64 --progress=plain \
            -t "taptarget/jupyter-server:$BUILD_TAG" \
            --no-cache=$NO_CACHE \
    ...
    if [ $IMG == "jupyter-enterprise-gateway" ]; then
        ...
        build=(docker build --platform linux/amd64 --progress=plain \
            -t "taptarget/jupyter-enterprise-gateway:$BUILD_TAG" \
            --no-cache=$NO_CACHE \
    ...
    if [ $IMG == "celery-worker" ]; then
        ...
        build=(docker build --platform linux/amd64 --progress=plain \
            -t "taptarget/celery-worker:$BUILD_TAG" \
            --no-cache=$NO_CACHE \
    ...
    if [ $IMG == "base-kernel-py" ]; then
        ...
        build=(docker build --platform linux/amd64 --progress=plain \
            -t "taptarget/base-kernel-py:$BUILD_TAG" \
    ...
    if [ $IMG == "base-kernel-julia" ]; then
        ...
        build_ctx=$DIR/../services/base-images
        build=(docker build --platform linux/amd64 --progress=plain \
            -t "taptarget/base-kernel-julia:$BUILD_TAG" \
    ...
    if [ $IMG == "base-kernel-javascript" ]; then
        ...
        build_ctx=$DIR/../services/base-images
        build=(docker build --platform linux/amd64 --progress=plain \
            -t "taptarget/base-kernel-javascript:$BUILD_TAG" \
    ...
    if [ $IMG == "base-kernel-r" ]; then
        ...
        build_ctx=$DIR/../services/base-images
        build=(docker build --platform linux/amd64 --progress=plain \
            -t "taptarget/base-kernel-r:$BUILD_TAG" \
    ...
    if [ $IMG == "orchest-api" ]; then
        ...
        build_ctx=$DIR/../services/orchest-api
        build=(docker build --platform linux/amd64 --progress=plain \
            -t "taptarget/orchest-api:$BUILD_TAG" \
            --no-cache=$NO_CACHE \
    ...
    if [ $IMG == "node-agent" ]; then
        ...
        build=(docker build --progress=plain \
            -t "taptarget/node-agent:$BUILD_TAG" \
            --no-cache=$NO_CACHE \
    ...
    if [ $IMG == "orchest-webserver" ]; then
        ...
        build=(docker build --platform linux/amd64 --progress=plain \
            -t "taptarget/orchest-webserver:$BUILD_TAG" \
            --no-cache=$NO_CACHE \
    ...
    if [ $IMG == "auth-server" ]; then
        ...
        build=(docker build --platform linux/amd64 --progress=plain \
            -t "taptarget/auth-server:$BUILD_TAG" \
            --no-cache=$NO_CACHE \
    ...
    if [ $IMG == "session-sidecar" ]; then
        ...
        build=(docker build --platform linux/amd64 --progress=plain \
            -t "taptarget/session-sidecar:$BUILD_TAG" \
            --no-cache=$NO_CACHE \
    ...
    if [ $IMG == "image-puller" ]; then
        ...
        build=(docker build --platform linux/amd64 --progress=plain \
            -t "taptarget/image-puller:$BUILD_TAG" \
            --no-cache=$NO_CACHE \
    ...
    if [ $IMG == "image-builder-buildx" ]; then
        ...
        build=(docker build --platform linux/amd64 --progress=plain \
            -t "taptarget/image-builder-buildx:$BUILD_TAG" \
            --no-cache=$NO_CACHE \
    ...
    if [ $IMG == "image-builder-buildkit" ]; then
        ...
        build=(docker build --platform linux/amd64 --progress=plain \
            -t "taptarget/image-builder-buildkit:$BUILD_TAG" \
            --no-cache=$NO_CACHE \
    ...
    if [ $IMG == "buildkit-daemon" ]; then
        ...
        build=(docker build --progress=plain \
            -t "taptarget/buildkit-daemon:$BUILD_TAG" \
            --no-cache=$NO_CACHE \
